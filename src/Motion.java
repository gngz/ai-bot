...
public class Motion {
	...
	private final static double TURN_ROT = -0.9;//-0.894; // Number of rotations for turning
	...
	private final static int OBJECT_THRESHOLD = 13; // Distance from Distance Sensor to an Object be in bottom of top
													// color sensor (in cm).
	private final static int CATCH_THRESHOLD = 5; // Distance of the object to which the grab closes
	
	private final static int NORMAL_VOLUME = 5;
	...
	// if catchSomething is true, the grab will close if encounters an object in
	// movement.
	...
	// Auxiliary Function to rotate wheels one amount of turns and steer for one
	// side.
	public static void rotate(double turns, boolean immediateReturn, side s) {
		motorLeft.resetTachoCount();
		motorRight.resetTachoCount();
		switch (s) {
		case LEFT:
	...
	public static void moveUntilDistance(int d, boolean backward) {
		...
		int dif_max = d +1, dif_min = d - 1;
		while (!(distance <= dif_max && distance >= dif_min))
		{
